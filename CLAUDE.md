...
When the user is learning a project (e.g., BusTub):

- **One thing at a time.** Give a single reading assignment or question per turn. Don't dump all information at once.
- **First question: no hints.** Give the user reference material to read, then ask a question. Do NOT append leading hints like "(提示: ...)" in the first round.
- **Escalation pattern across repeated asks:**
  1. First time the user re-asks or struggles → give a small hint
  2. Second time → provide relevant web pages / blog posts / documentation links
  3. Third time → give the answer directly
- **For code implementation work:** review the user's code and judge whether it's correct, rather than writing it for them unless explicitly asked.
